/*
 *  Rectangle_tests.h
    Description: Implements the tests for rectangles and RSS
...
#ifndef SRC_TEST_RECTANGLE_TESTS_H_
#define SRC_TEST_RECTANGLE_TESTS_H_
...
HOST_PREFIX float distRectangles_fcl(const float R[3][3], const float t[3],
                                     RSS s1, RSS s2) {
...
HOST_PREFIX void test_rectangles_2D() {
  ...
  std::cout << "RSS distance outside "
            << DIST_RSS(matI_, t0_, &p1, &q1, &preset_var) << std::endl;
  ...
  std::cout << "Rectangle distance on "
            << DIST_RSS(matI_, t0_, &p1, &q2, &preset_var) << std::endl;
  ...
  q3.l[1] = 3;
  q3.r = 0.0;
  std::cout << "Rectangle distance inside "
            << DIST_RSS(matI_, t0_, &p1, &q3, &preset_var) << std::endl;
  assert(approx_equal(distRectangles_fcl(matI_, t0_, p1, q3), 0));
  assert(approx_equal(DIST_RSS(matI_, t0_, &p1, &q3, &preset_var), 0));

  std::cout << "Rectangle distance inside "
            << DIST_RSS(matI_, t1_, &p1, &q1, &preset_var) << std::endl;
  std::cout << "Rectangle distance inside true"
            << distRectangles_fcl(matI_, t1_, p1, q1) << std::endl;
  assert(approx_equal(distRectangles_fcl(matI_, t1_, p1, q1), 2.5));
  assert(approx_equal(DIST_RSS(matI_, t1_, &p1, &q1, &preset_var), 2.5));

  std::cout << "Rectangle distance inside "
            << DIST_RSS(matI_, t2_, &p1, &q1, &preset_var) << std::endl;
  std::cout << "Rectangle distance inside true"
            << distRectangles_fcl(matI_, t2_, p1, q1) << std::endl;
  assert(approx_equal(distRectangles_fcl(matI_, t2_, p1, q1), 1.5 * sqrt(2)));
  assert(
      approx_equal(DIST_RSS(matI_, t2_, &p1, &q1, &preset_var), 1.5 * sqrt(2)));
...
HOST_PREFIX void test_rectangles_3D() {
  ...
  std::cout << "Rectangle distance outside "
            << DIST_RSS(matI_, t0_, &p1, &q1, &preset_var) << std::endl;
  ...
  assert(approx_equal(distRectangles_fcl(matI_, t0_, p1, q3), 0));
  std::cout << "Rectangle distance intersection "
            << DIST_RSS(matI_, t0_, &p1, &q3, &preset_var) << std::endl;
  assert(approx_equal(DIST_RSS(matI_, t0_, &p1, &q3, &preset_var), 0));
...
HOST_PREFIX void test_RSS_2D() {
  ...
  std::cout << "RSS distance outside " << distRectangles_fcl(matI_, t0_, p1, q1)
            << std::endl;
  assert(approx_equal(distRectangles_fcl(matI_, t0_, p1, q1), 0.5));
  std::cout << "RSS distance outisede "
            << DIST_RSS(matI_, t0_, &p1, &q1, &preset_var) << std::endl;
  ...
  q2.l[1] = 3;
  q2.r = 0.5;
  std::cout << "RSS distance on " << distRectangles_fcl(matI_, t0_, p1, q2)
            << std::endl;
  assert(approx_equal(distRectangles_fcl(matI_, t0_, p1, q2), 0));
  std::cout << "RSS distance on " << DIST_RSS(matI_, t0_, &p1, &q2, &preset_var)
            << std::endl;
  assert(approx_equal(DIST_RSS(matI_, t0_, &p1, &q2, &preset_var), 0));
  ...
  q3.l[1] = 3;
  q3.r = 0.5;
  std::cout << "RSS distance intersection "
            << distRectangles_fcl(matI_, t0_, p1, q3) << std::endl;
  assert(approx_equal(distRectangles_fcl(matI_, t0_, p1, q3), 0));
  std::cout << "RSS distance intersection "
            << DIST_RSS(matI_, t0_, &p1, &q3, &preset_var) << std::endl;
  assert(approx_equal(DIST_RSS(matI_, t0_, &p1, &q3, &preset_var), 0));
...
#endif  // SRC_TEST_RECTANGLE_TESTS_H_
