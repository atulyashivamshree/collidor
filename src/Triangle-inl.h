        ...
        const float S[3][3], const float T[3][3], TriDistVars* p_var)  
{
  ...
  VcrossV(p_var->Sn,p_var->Sv[0],p_var->Sv[1]); // Compute normal to S triangle
  p_var->Snl = VdotV(p_var->Sn,p_var->Sn);      // Compute square of length of normal
  ...
  if (p_var->Snl > 1e-15)  
  {
    ...
    {
      if (p_var->Tp[0] < p_var->Tp[1]) p_var->point = 0; else p_var->point = 1;
      ...
      if (VdotV(p_var->V,p_var->Z) > 0)
      {
        ...
        if (VdotV(p_var->V,p_var->Z) > 0)
        {
          ...
          if (VdotV(p_var->V,p_var->Z) > 0)
          {
            ...
            VpVxS(P,T[p_var->point],p_var->Sn,p_var->Tp[p_var->point]/p_var->Snl);
            VcV(Q,T[p_var->point]);
  ...
  if (p_var->Tnl > 1e-15)  
  {
    ...
    {
      if (p_var->Sp[0] < p_var->Sp[1]) p_var->point = 0; else p_var->point = 1;
    ...
    { 
      ...
      if (VdotV(p_var->V,p_var->Z) > 0)
      {
        ...
        if (VdotV(p_var->V,p_var->Z) > 0)
        {
          ...
          {
            VcV(P,S[p_var->point]);
            VpVxS(Q,S[p_var->point],p_var->Tn,p_var->Sp[p_var->point]/p_var->Tnl);
            return sqrt(VdistV2(P,Q));
          }
        }
      }
