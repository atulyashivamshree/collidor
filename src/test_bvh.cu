...
__host__ void test_MinReduction(Task *h_tasks)
{
  ...
  for(int i = 0; i < test_start.size(); i++)
  {
    ...
    for(int j = 0; j < num; j++)
    {
      ...
      if(min_dist < 0)
        cout << h_tasks[j + start].dist << "<0 : j " << j <<", j+start" << (j + start) << endl;
    }
    ...
    cudaMemcpy(h_min, d_min, MAX_BLOCKS_RED * sizeof(float), cudaMemcpyDeviceToHost);
    ...
    {
      cout << "[" << i <<"] EXP: " << min_dist << ", ACTUAL: " << min_dist_gpu << "start: " <<start << ", num: " << num << endl;
      count_fail++;
    }
  }
  ...
  if(!count_fail)
    ...
    cout << "test Minimum computation : FAILED" << count_fail << "/" << test_start.size() << endl;
}
...
__host__ void test_countTasks(Task *h_tasks)
{
  ...
  for(int i = 0; i < test_start.size(); i++)
  {
    ...
    cudaDeviceSynchronize();
    cudaMemcpy(h_count, d_count, MAX_BLOCKS_RED * sizeof(int), cudaMemcpyDeviceToHost);
    ...
    {
      cout << "[" << i <<"] EXP: " << count_actual << ", ACTUAL: " << count_gpu << "start: " <<start << ", num: " << num << endl;
      count_fail++;
    }
  }
  ...
  if(!count_fail)
    ...
    cout << "test Count computation : FAILED" << count_fail << "/" << test_start.size() << endl;
}