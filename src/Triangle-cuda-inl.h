...
__device__ __inline__ void deepCopy3x3(float target[3][3], const float mat[3][3])
{
  for(int i = 0; i < 3; i++)
    for(int j = 0; j < 3; j++)
      target[i][j] = mat[i][j];
...
__global__ void computeDistanceArray(const Triangle *arr_s1, const Triangle* arr_s2,
