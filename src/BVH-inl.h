/*
 *  BVH-inl.h
    Description: Implements the utility functions for Bounding Volume Hierarchy
...
#ifndef SRC_BVH_INL_H_
#define SRC_BVH_INL_H_
...
void printBVH(std::ostream& os, const BVH* bvh) {
  ...
  for (int i = 0; i < bvh->num_tri; i++) os << bvh->tri_arr[i];
}
...
void loadBVH(std::istream& is, BVH* bvh) {
  ...
  for (int i = 0; i < bvh->num_bv; i++) is >> bvh->bv_arr[i];

  for (int i = 0; i < bvh->num_tri; i++) is >> bvh->tri_arr[i];
}

#endif  // SRC_BVH_INL_H_
